...
# Triangulator release helper (npm)
...
phase_artifacts() {
  ...
  local tgz="triangulator-${VERSION}.tgz"
  mv "$packed" "$tgz"
...
phase_publish() {
  ...
  run "$RUNNER" npm view triangulator version
  run "$RUNNER" npm view triangulator time
}

phase_smoke() {
  banner "Smoke test in empty dir"
  local tmp=/tmp/triangulator-empty
  rm -rf "$tmp" && mkdir -p "$tmp"
  ( cd "$tmp" && npx -y triangulator@"$VERSION" "Smoke from empty dir" --dry-run )
}
...
Phases (run individually or all):
  ...
  smoke      empty-dir npx triangulator@<version> --dry-run
