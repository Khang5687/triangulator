...
describe('supportsOscProgress', () => {
  ...
  test('can be disabled with TRIANGULATOR_NO_OSC_PROGRESS', () => {
    // biome-ignore lint/style/useNamingConvention: env keys mirror real process env.
    expect(supportsOscProgress({ ...baseEnv, TRIANGULATOR_NO_OSC_PROGRESS: '1' }, true)).toBe(false);
  });
  ...
  test('force flag still requires TTY', () => {
    // biome-ignore lint/style/useNamingConvention: env keys mirror real process env.
    expect(supportsOscProgress({ ...baseEnv, TRIANGULATOR_FORCE_OSC_PROGRESS: '1' }, false)).toBe(false);
    // biome-ignore lint/style/useNamingConvention: env keys mirror real process env.
    expect(supportsOscProgress({ ...baseEnv, TRIANGULATOR_FORCE_OSC_PROGRESS: '1' }, true)).toBe(true);
  });
});
...
describe('startOscProgress', () => {
  test('emits OSC 9;4 sequences and clears on stop', () => {
    ...
    const stop = startOscProgress({
      ...
      targetMs: 2_000,
      // biome-ignore lint/style/useNamingConvention: env keys mirror real process env.
      env: { ...process.env, TRIANGULATOR_FORCE_OSC_PROGRESS: '1' },
      isTty: true,
