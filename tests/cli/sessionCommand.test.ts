...
describe('handleSessionCommand', () => {
  ...
  test('prints paths when --path is provided with an id', async () => {
    ...
    const getSessionPaths = vi.fn().mockResolvedValue({
      dir: '/tmp/.triangulator/sessions/abc',
      metadata: '/tmp/.triangulator/sessions/abc/meta.json',
      request: '/tmp/.triangulator/sessions/abc/request.json',
      log: '/tmp/.triangulator/sessions/abc/output.log',
    });
    ...
    expect(logSpy).toHaveBeenCalledWith('Session dir: /tmp/.triangulator/sessions/abc');
    expect(logSpy).toHaveBeenCalledWith('Metadata: /tmp/.triangulator/sessions/abc/meta.json');
    expect(logSpy).toHaveBeenCalledWith('Request: /tmp/.triangulator/sessions/abc/request.json');
    expect(logSpy).toHaveBeenCalledWith('Log: /tmp/.triangulator/sessions/abc/output.log');
    expect(process.exitCode).toBeUndefined();
  });
  ...
  test('rejects slug-style "clear" ids with guidance', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      'Session cleanup now uses --clear. Run "triangulator session --clear --hours <n>" instead.',
    );
    expect(process.exitCode).toBe(1);
  });
});
