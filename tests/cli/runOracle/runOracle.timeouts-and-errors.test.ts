...
describe('runOracle error handling', () => {
  test('surfaces prompt validation errors for short pro prompts', async () => {
    const original = process.env.TRIANGULATOR_MIN_PROMPT_CHARS;
    process.env.TRIANGULATOR_MIN_PROMPT_CHARS = '20';
    try {
      await expect(
        runOracle(
      ...
      if (original === undefined) {
        delete process.env.TRIANGULATOR_MIN_PROMPT_CHARS;
      } else {
        process.env.TRIANGULATOR_MIN_PROMPT_CHARS = original;
      }
    }
  });
