...
describe('triangulator --version', () => {
  test('prints the package.json version', async () => {
    const cliEntrypoint = path.join(process.cwd(), 'bin', 'triangulator-cli.ts');
    const { stdout, stderr } = await execFileAsync(process.execPath, ['--import', 'tsx', cliEntrypoint, '--version'], {
      // biome-ignore lint/style/useNamingConvention: environment variable name
      env: { ...process.env, FORCE_COLOR: '0', TRIANGULATOR_DISABLE_KEYTAR: '1' },
    });
