...
describe('bridge connection parsing', () => {
  ...
  it('parses triangulator+tcp://host:port?token=...', () => {
    const parsed = parseBridgeConnectionString('triangulator+tcp://example.com:1234?token=secret');
    expect(parsed).toEqual({ remoteHost: 'example.com:1234', remoteToken: 'secret' });
  });

  it('parses IPv6 hosts with brackets', () => {
    const parsed = parseBridgeConnectionString('triangulator+tcp://[2001:db8::1]:9473?token=abc');
  ...
  it('formats connection strings (with and without token)', () => {
    ...
    expect(withToken).toBe('triangulator+tcp://127.0.0.1:9473?token=abc');

    const withoutToken = formatBridgeConnectionString({ remoteHost: '127.0.0.1:9473', remoteToken: 'abc' }, { includeToken: false });
    expect(withoutToken).toBe('triangulator+tcp://127.0.0.1:9473');
  });
