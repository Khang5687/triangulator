...
const ENABLE_LIVE = process.env.TRIANGULATOR_LIVE_TEST === '1' || process.env.ORACLE_LIVE_TEST === '1';
const LIVE_API_KEY = process.env.OPENAI_API_KEY;

if (!ENABLE_LIVE || !LIVE_API_KEY) {
  describe.skip('OpenAI live smoke tests', () => {
    test('Set TRIANGULATOR_LIVE_TEST=1 with a real OPENAI_API_KEY to run these integration tests.', () => {});
  });
} else {
  const sharedDeps = {
    apiKey: LIVE_API_KEY,
