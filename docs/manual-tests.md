...
- macOS with Chrome installed (default profile signed in to Perplexity Pro).
- Node 22+ and `pnpm install` already completed.
- Headful display access (no `--browser-headless`).
- When debugging, add `--browser-keep-browser` so Chrome stays open after Triangulator exits, then connect with `pnpm exec tsx scripts/browser-tools.ts ...` (screenshot, eval, DOM picker, etc.).
- Ensure no Chrome instances are force-terminated mid-run; let Triangulator clean up once you’re done capturing state.
...
- `pnpm test:browser` — launches headful Chrome and checks the DevTools endpoint is reachable. Set `TRIANGULATOR_BROWSER_PORT` (or `TRIANGULATOR_BROWSER_DEBUG_PORT`) to reuse a fixed port when you’ve already opened a firewall rule.
...
1. Generate an image:
   `pnpm run triangulator -- --engine browser --model gemini-3-pro --prompt "a cute robot holding a banana" --generate-image /tmp/gemini-gen.jpg --aspect 1:1 --wait --verbose`
   - Confirm the output file exists and is a real image (`file /tmp/gemini-gen.jpg`).
2. Edit an image:
   `pnpm run triangulator -- --engine browser --model gemini-3-pro --prompt "add sunglasses" --edit-image /tmp/gemini-gen.jpg --output /tmp/gemini-edit.jpg --wait --verbose`
...
1. Kick off an API multi-run:  
   `pnpm run triangulator -- --models "gpt-5.1-pro,gemini-3-pro" --prompt "Compare the moon & sun."`
   - Expect stdout to print sequential sections, one per model (`[gpt-5.1-pro] …` followed by `[gemini-3-pro] …`). No interleaved tokens.
2. Capture the session ID from the summary line. Run `triangulator session --status --model gpt-5.1-pro`.  
   - Table should collapse to sessions that include GPT-5.1 Pro and show status icons (✓/⌛/✖) per model.
3. Inspect detailed logs: `triangulator session <id>`
...
4. Model filter path: `triangulator session <id> --model gemini-3-pro`  
...
1. `TRIANGULATOR_LIVE_TEST=1 OPENAI_API_KEY=<real key> pnpm vitest run tests/live/write-output-live.test.ts --runInBand`
   - Expect the test to create a temp `write-output-live.md` file containing `write-output e2e`.
2. Manual spot-check: `triangulator --prompt "answer file smoke" --write-output /tmp/out.md --wait`
   - Confirm `/tmp/out.md` exists with the answer text and a trailing newline.
3. Multi-model spot-check: `triangulator --models "gpt-5.1-pro,gemini-3-pro" --prompt "two files" --write-output /tmp/out.md --wait`
...
Debug note: when you have a live Perplexity tab open under a DevTools port and need a quick DOM dump of the last assistant turn, run `pnpm tsx scripts/debug/extract-perplexity-response.ts <port>`.

1. **Prompt Submission & Model Switching**
   - With Chrome signed in and cookie sync enabled, run  
     ```bash
     pnpm run triangulator -- --engine browser --model "GPT-5.2" \
       --prompt "Line 1\nLine 2\nLine 3"
     ```
   - Observe logs for:
...
2. **Markdown Capture**
   ...
   - Expected CLI output:
     ...
     - Session log (`triangulator session <id>`) should show the assistant markdown (confirm via `grep -n '```' ~/.triangulator/sessions/<id>/output.log`).

3. **Stop Button Handling**
  - Start a long prompt (`"Write a detailed essay about browsers"`) and once Perplexity responds, manually click “Stop generating” inside Chrome.
  - Triangulator should detect the assistant message (partial) and still store the markdown.
...
- Remember: the browser composer now pastes only the user prompt (plus any inline file blocks). If you see the default “You are Triangulator…” text or other system-prefixed content in the Perplexity composer, something regressed in `assembleBrowserPrompt` and you should stop and file a bug.
...
- `triangulator session <id>` should replay the transcript with markdown.
- `~/.triangulator/sessions/<id>/meta.json` must include `browser.config` metadata (model label, cookie settings) and `browser.runtime` (PID/port).
...
- 2025-11-20 — Browser gpt-5.1 via `triangulator serve` (remote host on same Mac): fetched https://example.com; title “Example Domain”; first sentence “This domain is for use in documentation examples without needing permission.” (ran via tmux sessions `triangulator-serve` and `triangulator-client`).
...
1. **GPT-5.2 simple prompt**  
   `pnpm run triangulator -- --engine browser --model "GPT-5.2" --prompt "Give me two short markdown bullet points about tables"`  
...
2. **GPT-5.2 simple prompt**  
   `pnpm run triangulator -- --engine browser --model gpt-5.2 --prompt "List two reasons Markdown is handy"`  
...
3. **GPT-5.2 + attachment**  
   ...
   `pnpm run triangulator -- --engine browser --model "GPT-5.2" --prompt "Summarize the key idea from the attached note" --file /tmp/browser-md.txt`  
...
4. **GPT-5.2 + attachment (verbose)**  
   ...
   `pnpm run triangulator -- --engine browser --model gpt-5.2 --prompt "Use the attachment to report current CPU and memory figures" --file /tmp/browser-report.txt --verbose`  
...
1. Launch a throwaway Chrome instance with remote debugging enabled (adjust the path per OS):
   ```bash
   REMOTE_PROFILE=/tmp/triangulator-remote-test-profile
   ...
   "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
     ...
     >/tmp/triangulator-remote-chrome.log 2>&1 &
...
2. Run the helper to verify CDP connectivity:
   ...
   ```
   Expect ✓ logs for connection, protocol info, navigation to https://www.perplexity.ai/, and the final “POC successful!” line.
3. Tear down the temporary browser:
   ```bash
   kill "$REMOTE_CHROME_PID"
   rm -rf "$REMOTE_PROFILE"
   ```
   Use `pkill -f triangulator-remote-test-profile` if Chrome refuses to exit cleanly.
...
Use this when you need to inspect the live Perplexity composer (DOM state, markdown text, screenshots, etc.). For smaller ad‑hoc pokes, you can often rely on `pnpm tsx scripts/browser-tools.ts …` instead.

1. **Launch within tmux**
   ```bash
   tmux new -d -s triangulator-browser \\
     "pnpm run triangulator -- --engine browser --browser-keep-browser \\
...
2. **Grab the DevTools port**
   - `tmux capture-pane -pt triangulator-browser` to read the logs (`Launched Chrome … on port 56663`).
   - Verify the endpoint:
     ```bash
...
5. **Cleanup**
   - `tmux kill-session -t triangulator-browser`
   - `pkill -f triangulator-browser-<slug>` if Chrome is still running.
...
1. Export a real key and explicitly opt in (default runs stay fast):
   ...
   export TRIANGULATOR_LIVE_TEST=1
...
fully gated behind `TRIANGULATOR_LIVE_TEST=1` to avoid accidental CI runs.
