...
- Cookies: cookie sync is disabled by default on Windows because Perplexity cookies are app-bound (`v20`) and fail decryption. Use `--browser-manual-login` to reuse a persistent automation profile and sign in once (skips cookie copy entirely). Inline cookies remain available (`--browser-inline-cookies(-file)` / `TRIANGULATOR_BROWSER_COOKIES_JSON`).
- Manual login flow: run with `--browser-manual-login` and sign into www.perplexity.ai in the opened Chrome; Triangulator waits until the session is active. For initial login/setup or debugging, add `--browser-keep-browser` to keep the window open after the run; otherwise Triangulator closes Chrome but preserves the profile at `~/.triangulator/browser-profile` (override with `TRIANGULATOR_BROWSER_PROFILE_DIR` or `browser.manualLoginProfileDir` in `~/.triangulator/config.json`). If that automation Chrome is already running with remote debugging enabled (DevToolsActivePort present), reuse it instead of relaunching by pointing Triangulator at it via `--remote-chrome <host:port>`.
