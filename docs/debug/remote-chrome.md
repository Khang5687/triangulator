...
- Goal: Run Triangulator browser mode from local laptop against VM-hosted Chrome via new `triangulator serve` remote service.
- VM IP: 192.168.64.2
...
- Later run reached remote path but failed model switch: `Unable to find model option matching "GPT-5.2 Pro"` (remote Chrome not logged into Perplexity / model picker mismatch).
- After disabling cookie shipping and requiring host login, remote runs now fail earlier: service logs “Loading Perplexity cookies from host Chrome profile…” then reports `Unhandled promise rejection ... Unknown error` when `loadChromeCookies` runs on the VM. Remote client sees `socket hang up` because the server doesn’t deliver a result.
...
- `./runner` requires bun; starting service with:
  ```
  cd ~/Projects/triangulator
  export PATH="$HOME/.bun/bin:$PATH"
  ./runner pnpm run triangulator -- serve --port 49810 --token cd93955b64d5afcb946a4a4a89651313
  ```
...
- Remote model switch failed: remote Chrome likely not signed into Perplexity; model picker couldn’t find “GPT-5.2 Pro”.
- Keychain/cookie read now failing on VM: `loadChromeCookies` throws “Unknown error” when invoked from the server process (Node 25, SSH shell). When `triangulator serve` runs from GUI Terminal it starts fine; under nohup/SSH it logs the rejection and remote runs hang.
- New behavior (post-fix): `triangulator serve` exits early if it cannot load host Perplexity cookies after opening www.perplexity.ai for login; sign in on the host and restart the service.
...
- Sign into Perplexity in the VM’s Chrome profile used by the service so model switching succeeds. (Currently we rely on host cookies only; client cookie shipping is disabled.)
- If cookie loading keeps failing under SSH/nohup, start `triangulator serve` from a GUI macOS session or switch to Node 20 to avoid Keychain issues. The service now exits early after opening www.perplexity.ai when no cookies are present—log in, then restart it.
- Retry local command above; ensure service logs show incoming /runs and that the login probe passes (no login button).
- Optional: switch to a fresh port/token (`triangulator serve` with no args) to avoid lingering listeners.
