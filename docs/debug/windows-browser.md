...
Triangulator reads Chrome cookies via `@steipete/sweet-cookie` (uses `node:sqlite` + PowerShell DPAPI on Windows).

Notes:
- Perplexity cookies may be app-bound (`v20`) and can still fail to decrypt depending on the machine/account.
