...
### Added
- Bridge: add the bridge workflow + MCP browser controls for remote Perplexity sessions. Original PR #42 by Kyle McCleary (@kmccleary3301) — thank you!
...
### Fixed
- Browser: fix attachment uploads in the current Perplexity composer (avoid duplicate uploads; avoid image-only inputs for non-image files). Original PR #60 by Alex Naidis (@TheCrazyLex) — thank you!
...
- Browser: dismiss blocking modals when a custom Perplexity project URL is missing, and harden attachment uploads (force input/change events; retry via DataTransfer; treat “file selected” as insufficient unless the composer shows attachment UI).
- Browser: prefer a trusted (CDP) click on the composer “+” button so attachment uploads work even when Perplexity ignores synthetic clicks.
...
### Added
- Config: allow `browser.thinkingTime`, `browser.manualLogin`, and `browser.manualLoginProfileDir` defaults in `~/.triangulator/config.json`.
...
- Browser: when a custom Perplexity project URL is missing, retry on the base URL with a longer prompt timeout.
...
- New `--browser-model-strategy` flag to control Perplexity model selection (`select`/`current`/`ignore`) in browser mode. Original PR #49 by @djangonavarro220 — thank you!
...
- Login flow adds richer diagnostics, auto-accepts the “Welcome back” picker, and always logs the active Perplexity URL.
- Cookie handling prefers live Chrome over legacy `~/.triangulator/cookies.json`; Gemini web can use inline cookies when sync is disabled.
...
### Changed
- Browser: add `--browser-thinking-time <light|standard|extended|heavy>` to select thinking-time intensity in Perplexity.
...
- Browser: attachment uploads re-trigger the file-input change event until Perplexity renders the attachment card (avoids hydration races); verify attachments are present on the sent user message before waiting for the assistant.
...
- Browser: browser engine GPT selection now supports Perplexity 5.2 (`gpt-5.2`) and Perplexity 5.2 Pro (`gpt-5.2-pro`); legacy labels like `gpt-5.1` normalize to 5.2, and “Pro” always resolves to 5.2 Pro (ignores Legacy GPT‑5.1 Pro submenu) with a top-bar label confirmation.
...
- Browser: adapt to Perplexity DOM changes (`data-turn=assistant|user`) and “Answer now” gating in Pro thinking so we don’t capture placeholders/truncate answers.
...
### Changed
- Browser: default model target now prefers Perplexity 5.2. Original PR #40 by Muly Oved (@mulyoved) — thank you!
...
- Browser: new `--browser-attachments <auto|never|always>` (default `auto`) pastes file contents inline up to ~60k characters, then switches to uploads; if Perplexity rejects an inline paste as too large, Triangulator retries automatically with uploads.
  - Note: the ~60k threshold is based on pasted **characters** in the Perplexity composer (not token estimates); on rejection we log the retry and switch to uploads automatically.
...
### Changed
- Docs: README now explicitly warns against `pnpx triangulator` (pnpx cache breaks sqlite bindings); use `npx -y triangulator` instead. Thanks Xuanwo for flagging this.
...
- Browser uploads: detect Perplexity’s composer attachment chip (not echoed in the last user turn) to avoid false “Attachment did not appear” failures. Thanks Mariano Belinky (@mbelinky) for the fix.
- Browser interruption: if the user/agent sends SIGINT/SIGTERM/SIGQUIT while the assistant response is still pending, Triangulator leaves Chrome running, writes runtime hints, and logs how to reattach with `triangulator session <slug>` instead of killing the browser mid-run.
- Browser uploads (Perplexity UI 2025-12): wait for DOM ready, avoid duplicate uploads, and block Send until the attachment chip/file name (or “N files” pill) is visible so files aren’t sent empty or multiple times.
...
### Changed
- `triangulator` with no arguments now prints the help/usage banner; launch the interactive UI explicitly via `triangulator tui` (keeps `TRIANGULATOR_FORCE_TUI` for automation/tests). README updated to match.
...
### Fixed
- Browser config defaults from `~/.triangulator/config.json` now apply when CLI flags are untouched (chromePath/profile/cookiePath), fixing “No Chrome installations found” when a custom browser path is configured.
- Browser engine now verifies each attachment shows up in the composer before sending (including remote/serve uploads), fixing cases where file selection succeeded but Perplexity never received the files (e.g., WKWebView blank runs).

## 0.5.1 — 2025-12-03

### Added
- Browser runs now auto-click the Perplexity “Answer now” gate after sending, so workspace prompts continue without manual intervention.

### Changed
- `triangulator status` uses the same session table formatting as the TUI (status/model/mode/timestamp/chars/cost/slug) for consistent layout.
...
### Added
- Browser sessions now persist Chrome reattach hints (port/host/target/url) and log them inline; `triangulator session <id>` can reconnect to a live tab, harvest the assistant turn, and mark the run completed even if the original controller died. Includes a reconnection helper and regression tests for runtime hint capture and reattach.
...
### Fixed
- CLI/TUI now print the intro banner only once; forced TUI launches (`TRIANGULATOR_FORCE_TUI` or no args in a TTY) no longer show duplicate 🧿 header lines.
- TUI session list cleans up separators, removing the `__disabled__ (Disabled)` placeholder and `(Disabled)` tag on the header row.
- `triangulator session --render` no longer drops answers when the model filter is empty or per-model logs are missing (common for browser runs); stored session output is rendered again.
- Browser uploads no longer time out in Perplexity project workspaces: file input/send-button selectors are broader, upload completion falls back to attached files when buttons are missing, and we added tests to guard the new selectors.
...
- Browser reattach flow survives controller loss: the controller PID is persisted with the Chrome port/URL so `triangulator session <id>` can reconnect, harvest the assistant turn, and mark the run completed even if the original process died.
...
### Added
- `--browser-manual-login` skips cookie copy, reuses a persistent automation profile (`~/.triangulator/browser-profile` by default), and waits for manual Perplexity login—handy on Windows where app-bound cookies can’t be decrypted; works as an opt-in on macOS/Linux too.
- Manual-login browser sessions can reuse an already-running automation Chrome when remote debugging is enabled; point Triangulator at it via `--remote-chrome <host:port>` to avoid relaunching/locks.
- `--browser-port` (alias `--browser-debug-port`, env `TRIANGULATOR_BROWSER_PORT`) pins the DevTools port so WSL/Windows users can open a single firewall rule; includes a lightweight `pnpm test:browser` DevTools reachability check.
...
- Publish metadata now declares Node >=20 (engines/devEngines) and drops the implicit bun runtime so `npx triangulator` no longer fails with EBADDEVENGINES on newer Node versions.
...
- Grok “thinking”/non-fast variant is not available via API yet; Triangulator aliases `grok` to the fast reasoning model to match what xAI ships today.
- PTY-driven CLI/TUI harness landed for e2e coverage (browser guard, TUI exit path); PTY suites are opt-in via `TRIANGULATOR_ENABLE_PTY_TESTS=1` and stub tokenizers to stay lightweight.

### Fixed
- MCP (global installs): keep the stdio transport alive until the client closes it so `triangulator-mcp` doesn’t exit right after `connect()`; npm -g / host-spawned MCP clients now handshake successfully (tarball regression in 0.4.2).

## 0.4.2 — 2025-11-21

### Fixed
- MCP: `npx triangulator triangulator-mcp` now routes directly to the MCP server (even when npx defaults to the CLI binary) and keeps stdout JSON-only for Cursor/other MCP hosts.
- Added the missing `@anthropic-ai/tokenizer` runtime dependency so `npx triangulator triangulator-mcp` starts cleanly.
...
- Markdown bundles render better in the CLI and Perplexity: each attached file now appears as `### File: <path>` followed by a fenced code block (language inferred), across API bundles, browser bundles (including inline mode), and render/dry-run output; ANSI highlighting still applies on rich TTYs.
...
- Browser engine: `--perplexity-url` (alias `--browser-url`) and `browser.perplexityUrl` config let you target specific Perplexity Spaces URLs while keeping API `--base-url` separate.
...
- Duplicate prompt guard remains active: Triangulator blocks a second run when the exact prompt is already running.

### Changed
- Cookie sync covers Chrome, Chromium, Edge, Brave, and Vivaldi profiles; targets www.perplexity.ai. Windows browser automation is still partial—prefer API or clipboard fallback there.
- Reject prompts shorter than 10 characters with a friendly hint for pro-tier models (`gpt-5.1-pro`) only (prevents accidental costly runs while leaving cheaper models unblocked). Override via TRIANGULATOR_MIN_PROMPT_CHARS for automated environments.
...
- **Gemini 3 Pro Support**: Use Google's latest model via `triangulator --model gemini`. Requires `GEMINI_API_KEY`.
...
- Browser inline cookies: `--browser-inline-cookies[(-file)]` (or env) accepts JSON/base64 payloads, auto-loads `~/.triangulator/cookies.{json,base64}`, adds a cookie allowlist (`--browser-cookie-names`), and dry-run now reports whether cookies come from Chrome or inline sources.
...
### Changed
 ...
 - Browser fallback tips focus on `--browser-bundle-files`, making it clear users can drag the single bundled file into Perplexity when automation fails.
 - Sessions TUI separates recent vs older runs, adds an Older/Newer action, keeps headers aligned with rows, and avoids separator crashes while preserving an always-selectable “ask triangulator” entry.
...
### Added
- `triangulator-mcp` stdio server (bin) with `consult` and `sessions` tools plus read-only session resources at `triangulator-session://{id}/{metadata|log|request}`.
...
- Per-user JSON5 config at `~/.triangulator/config.json` to set default engine/model, notification prefs (including sound/mute rules), browser defaults, heartbeat, file-reporting, background mode, and prompt suffixes. CLI/env still override config.
...
- Default ignores when crawling project roots now drop common build/cache folders (`node_modules`, `dist`, `coverage`, `.git`, `.turbo`, `.next`, `build`, `tmp`) unless the path is passed explicitly. Triangulator logs each skipped path for transparency.
...
- macOS notifier helper now ships signed/notarized with the Triangulator icon and auto-repairs execute bits for the fallback terminal-notifier.
...
- Help banner and first tip now require “prompt + --file” (dirs/globs fine) and remind you Triangulator can’t see your project without attachments.
...
- Claude Code (global/user scope):  
  `claude mcp add --scope user --transport stdio triangulator -- triangulator-mcp`
- Project-scoped Claude: drop `.mcp.json` next to the repo root with
  ```json
  {
    "mcpServers": {
      "triangulator": { "type": "stdio", "command": "npx", "args": ["-y", "triangulator", "triangulator-mcp"] }
    }
  }
  ```
- The MCP `consult` tool honors `~/.triangulator/config.json` defaults (engine/model/search/prompt suffix/heartbeat/background/filesReport) unless the caller overrides them.
...
Highlights
- Markdown rendering for completed sessions (`triangulator session|status <id> --render` / `--render-markdown`) with ANSI formatting in rich TTYs; falls back to raw when logs are huge or stdout isn’t a TTY.
- New `--path` flag on `triangulator session <id>` prints the stored session directory plus metadata/request/log files, erroring if anything is missing. Uses soft color in rich terminals for quick scanning.

Details
### Added
- `triangulator session <id> --path` now prints the on-disk session directory plus metadata/request/log files, exiting with an error when any expected file is missing instead of attaching.
- When run in a rich TTY, `--path` labels and paths are colorized for easier scanning.

### Improved
- `triangulator session|status <id> --render` (alias `--render-markdown`) pretty-prints completed session markdown to ANSI in rich TTYs, falls back to raw when non-TTY or oversized logs.
## 0.0.10 — 2025-11-17

### Added
- Rich terminals that support OSC 9;4 (Ghostty 1.2+, WezTerm, Windows Terminal) now show an inline progress bar while Triangulator waits for the OpenAI response; disable with `TRIANGULATOR_NO_OSC_PROGRESS=1`, force with `TRIANGULATOR_FORCE_OSC_PROGRESS=1`.
...
### Changed
- Help tips call out that Triangulator is one-shot and does not remember prior runs, so every query should include full context.
- `triangulator session <id>` now logs a brief notice when extra root-only flags are present (e.g., `--render-markdown`) to make it clear those options are ignored during reattach.

## 0.0.7 — 2025-11-16

### Changed
- Browser-mode thinking monitor now emits a text-only progress bar instead of the "Pro thinking" string.
- `triangulator session <id>` trims preamble/log noise and prints from the first `Answer:` line once a session is finished.
...
### Browser mode
- When more than 10 files are provided, automatically bundles all files into a single `attachments-bundle.txt` to stay under Perplexity’s upload cap and logs a verbose warning when bundling occurs.
...
### Added
- Positional prompt shorthand: `triangulator "prompt here"` (and `npx -y triangulator "..."`) now maps the positional argument to `--prompt` automatically.

### Fixed
- `triangulator status/session` missing-prompt guard now coexists with the positional prompt path and still shows the cleanup tip when no sessions exist.

## 0.0.1 — 2025-11-15

### Fixed
- Corrected npm binary mapping so `triangulator` is installed as an executable. Published with `--tag beta`.
...
### Fixed
- `triangulator status` and `triangulator session` no longer demand `--prompt` when used directly.
