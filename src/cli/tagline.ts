...
const TAGLINES = [
  ...
  'Token thrift, triangulator lift.',
  ...
  'Debugger by day, triangulator by night.',
  ...
  'When the other agents shrug, the triangulator ships.',
  ...
  'Model said maybe; triangulator says ship/no.',
  'Less lorem, more logic.',
  "Your prompt's adult supervision.",
  'Cleanup crew for AI messes.',
  'AI wrote it? Triangulator babysits it.',
  ...
  'Triangulator babysits; you merge.',
  ...
  'New year, same triangulator: resolutions shipped, not wished.',
  ...
  "Workers' Day: let triangulator haul the heavy context.",
...
const HOLIDAY_RULES = new Map<string, HolidayRule>([
  ['New year, same triangulator: resolutions shipped, not wished.', onMonthDay(0, 1)],
  [
    'Lunar New Year sweep: clear caches, invite good deploys.',
    onSpecificDates(
      [
  ...
  ["Workers' Day: let triangulator haul the heavy context.", onMonthDay(4, 1)],
...
export function pickTagline(options: TaglineOptions = {}): string {
  ...
  const override = env?.TRIANGULATOR_TAGLINE_INDEX ?? env?.ORACLE_TAGLINE_INDEX;
...
export function formatIntroLine(version: string, options: TaglineOptions = {}): string {
  ...
  if (rich && chalk.level > 0) {
    return `${chalk.bold('🧿 triangulator')} ${version} — ${tagline}`;
  }
  return `🧿 triangulator ${version} — ${tagline}`;
}
