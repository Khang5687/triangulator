...
const CLEANUP_TIP =
  'Tip: Run "triangulator session --clear --hours 24" to prune cached runs (add --all to wipe everything).';
...
export async function showStatus({
  ...
  if (truncated) {
    ...
    console.log(
      chalk.yellow(
        `Showing ${entries.length} of ${total} sessions from the requested range. Run "triangulator session --clear" or delete entries in ${sessionsDir} to free space, or rerun with --status-limit/--status-all.`,
      ),
    );
  }
  if (showExamples) {
...
export async function attachSession(sessionId: string, options?: AttachSessionOptions): Promise<void> {
  ...
  const isVerbose = Boolean(process.env.TRIANGULATOR_VERBOSE_RENDER ?? process.env.ORACLE_VERBOSE_RENDER);
...
function printStatusExamples(): void {
  ...
  console.log(`${chalk.bold('  triangulator status --hours 72 --limit 50')}`);
  console.log(dim('    Show 72h of history capped at 50 entries.'));
  console.log(`${chalk.bold('  triangulator status --clear --hours 168')}`);
  console.log(dim('    Delete sessions older than 7 days (use --all to wipe everything).'));
  console.log(`${chalk.bold('  triangulator session <session-id>')}`);
