...
export async function shouldBlockDuplicatePrompt({
  ...
  log(
    chalk.yellow(
      `A session with the same prompt is already running (${duplicate.id}). Reattach with "triangulator session ${duplicate.id}" or rerun with --force to start another run.`,
    ),
  );
  return true;
}
