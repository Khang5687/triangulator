...
export async function handleSessionCommand(
  ...
  if (sessionOptions.verboseRender) {
    process.env.TRIANGULATOR_VERBOSE_RENDER = '1';
  }
  ...
  if (sessionId === 'clear' || sessionId === 'clean') {
    console.error('Session cleanup now uses --clear. Run "triangulator session --clear --hours <n>" instead.');
    process.exitCode = 1;
    return;
  }
  if (pathRequested) {
...
export function formatSessionCleanupMessage(
  ...
  const hint = 'Run "triangulator session --clear --all" to delete everything.';
