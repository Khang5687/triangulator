...
function renderHelpFooter(program: Command, colors: HelpColors): string {
  const tips = [
    ...
    `${colors.bullet('•')} Triangulator is one-shot: it does not remember prior runs, so start fresh each time with full context.`,
    `${colors.bullet('•')} Run ${colors.accent('--files-report')} to inspect token spend before hitting the API.`,
    `${colors.bullet('•')} Non-preview runs spawn detached sessions (especially gpt-5.2-pro API). If the CLI times out, do not re-run — reattach with ${colors.accent('triangulator session <slug>')} to resume/inspect the existing run.`,
    ...
    `${colors.bullet('•')} If any Triangulator session is already running, do not start new API runs. Attach to the existing browser session instead; only trigger API calls when you explicitly mean to.`,
  ...
  const examples = [
    formatExample(
      ...
      'Build the bundle, print it, and copy it for manual paste into Perplexity.',
    ),
    formatExample(
